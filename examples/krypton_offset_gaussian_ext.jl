...
H0_func = z -> 0.3 + MagneticField.gaussian_Br(z, 1.0, 0.75, 0.20)
...
const E0_DIMLESS = PaperScales.E0_dimless_from_discharge_voltage(
    U_DISCHARGE_V, scales, HEXT_DIM_MAX; L_m = L_M,
)
...
params = PartCount.SimParams(;
    L = 1.0, M = 80, mi = 1.0, me = me_ratio, T_ion = 1.0, v_a = 0.01, n_a_left = 60.0,
    ...
    collision_model = :spitzer, alpha_B = 0.0, E_z0_dimless = E0_DIMLESS, pic_charge_factor = 1.0,
    include_self_B = true)
...
CoreSolver.run_simulation(
    ...
    do_plot = true,
    plot_output_dir = joinpath(ROOT, "output", "figures", "krypton_offset_gaussian_ext"),
