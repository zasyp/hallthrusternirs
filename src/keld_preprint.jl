...
# Linear interpolation weights (with boundary protection)
...
# Neutral density update
function update_neutrals(n_a_new::Vector{Float64}, n_a_old::Vector{Float64},
    ...
    for k in 2:M+1
        ...
        n_a_new[k] = max(term1 + term2, 0.0)
    end
    return n_a_new
end

# Compute current j from H (Ampere's law)
...
# Solve Ey (generalized Ohm + Faraday law)
function solve_electric_field!(E_y::Vector{Float64}, H_x::Vector{Float64}, H0::Float64,
    ...
    M = length(H_x)
    @assert length(E_y) == M+1 && length(n) == M+1 && length(vz) == M+1 && length(T) == M+1 && length(j_old) == M+1
    H_int = zeros(M+1)
    H_int[1] = H_x[1]
    for k in 2:M
    ...
    for k in 2:M
        ...
        if n_safe > 0 && T_safe > 0
            coeff = α / (n_safe * h^2)
            a[k] = -coeff; b[k] = 1.0 + 2.0 * coeff; c[k] = -coeff
            H_star_k = H_int[k] + H0
    ...
    if bc_type == :j0
        ...
        d[2] -= a[2] * E_y[1]; a[2] = 0.0
        d[M] -= c[M] * E_y[M+1]; c[M] = 0.0
    else
    ...
    if N_inner > 0
        ...
        for i in 1:N_inner
            k = i+1
            b_inner[i] = b[k]; d_inner[i] = d[k]
            i < N_inner && (c_inner[i] = c[k])
            i > 1 && (a_inner[i-1] = a[k])
        end
...
# Explicit Ez formula
function compute_Ez!(Ez::Vector{Float64}, H_x::Vector{Float64}, H0::Float64,
                     ...
                     N_REG::Float64 = 0.02, νE::Float64 = 0.08)
    M = length(H_x)
    @assert length(Ez) == M+1
    n_s = copy(n); T_s = copy(T)
    smooth_field!(n_s, 2); smooth_field!(T_s, 2)
    H_star = similar(Ez)
    H_star[1] = H_x[1] + H0
    for k in 2:M
...
function compute_grid_quantities!(particles, x_grid, n, v_y, v_z, T, h)
    M = length(x_grid) - 1
    fill!(n, 0.0)
    n_v_y = zeros(length(n)); n_v_z = zeros(length(n)); n_T = zeros(length(n))
    for p in particles
        p.active || continue
        k0, k1, w0, w1 = interpolation_weights(p.z, x_grid)
        n[k0] += p.q * w0; n_v_y[k0] += p.q * w0 * p.vy; n_v_z[k0] += p.q * w0 * p.vz; n_T[k0] += p.q * w0 * p.T
        n[k1] += p.q * w1; n_v_y[k1] += p.q * w1 * p.vy; n_v_z[k1] += p.q * w1 * p.vz; n_T[k1] += p.q * w1 * p.T
    end
    for k in eachindex(n)
        ...
        if n[k] > 1e-12
            ...
            v_y[k] = v_z[k] = 0.0; T[k] = 1e-12
        end
    end
end

# Particle motion (predictor-corrector)
function move_particles!(particles, E_y0, E_y1, E_z0, E_z1, H_x0, H_x1, j0, j1,
                         ν_m0_grid, ν_m1_grid, x_grid, x_half, τ, h, ε, ν_m0_const,
                         v_a, kI, n_a_new, H0_func)
    thrust_step = 0.0
    L = x_grid[end]
    for p in particles
        p.active || continue
        z = p.z; vy = p.vy; vz = p.vz
        ...
        for i in 1:N0
            ...
            if z <= x_half[1]
                kh0, kh1, wh0, wh1 = 1, 1, 1.0, 0.0
            elseif z >= x_half[end]
                kh0, kh1, wh0, wh1 = length(x_half), length(x_half), 1.0, 0.0
            else
                kh0 = clamp(floor(Int, (z - x_half[1]) / h) + 1, 1, length(x_half)-1)
                wh1 = (z - x_half[kh0]) / h; wh0 = 1.0 - wh1; kh1 = kh0 + 1
            end
            ...
            vy_new = vy + τ0 * (ε * (E_y_mid + H_star_mid*vz_pred - ν_m_mid*j_mid) - ν_m_mid * vy)
            vz_new = vz + τ0 * (ε * (E_z_mid - H_star_mid*vy_pred) - ν_m_mid * vz)
            if isnan(vy_new) || isnan(vz_new) || abs(vy_new) > 1e6 || abs(vz_new) > 1e6
                p.active = false; break
            end
            z_new = clamp(z + τ0 * (vz + vz_new)/2, x_grid[1], x_grid[end])
            p.z = z_new; p.vy = vy_new; p.vz = vz_new
            z = z_new; vy = vy_new; vz = vz_new
        end
        if p.z >= L
            p.active = false
...
# Add new particles from ionization
function add_new_particles!(particles, n_a_new, n_ion, x_grid, τ, kI, v_a, T_ion, h)
    for k in eachindex(x_grid)
        ...
        q_new < 1e-6 && continue
        push!(particles, Particle(x_grid[k], 0.0, v_a, T_ion, q_new, true))
    end
end

# Remove inactive particles (boundary + recombination)
...
function plot_results(snapshots, thrust_time, thrust_values, save_times)
    if !isempty(snapshots)
        ...
        savefig("profiles.png"); display(current())
    end
    if !isempty(thrust_time)
        p_thrust = plot(thrust_time, thrust_values, xlabel="t", ylabel="F_T", legend=false)
        savefig("thrust.png"); display(p_thrust)
    end
end

# Intermediate electron temperature
function intermediate_temperature(T_new::Vector{Float64}, T_old::Vector{Float64},
                                  n::Vector{Float64}, vz::Vector{Float64}, j::Vector{Float64},
                                  n_a::Vector{Float64}, τ::Float64, γ::Float64,
                                  mi_over_mΣ::Float64, ν_m0::Float64, kI::Float64, h::Float64)
    M = length(T_old) - 1
    for k in 2:M
        n_safe = max(n[k], 0.01)
    ...
    T_new[1] = T_new[2]; T_new[M+1] = T_new[M]
    return T_new
end

# Main simulation
function run_simulation(;
    L=1.0, M=100, N1=1000, T_ion=1.0, v_a=0.1, n_a_left=10.0, kI=1.0,
    ε=1.0, ν_m0=15.0, γ=5/3, mi_over_mΣ=1.0, H0_func=z->1.0,
    α=0.75^2, α0=367.0, ζ=0.061, ε_dim=1.0,
    ...
    while t < total_time
        ...
        H0_val = H0_func(0.0)
        E_y, H_x_half, j = solve_electric_field!(E_y, H_x_half, H0_val, n_ion, v_iz, T_e, j, τ, α, ν_m0, h, :j0)
        smooth_field!(E_y, 15); smooth_field!(H_x_half, 15); smooth_field!(j, 10)
        compute_Ez!(E_z, H_x_half, H0_val, n_ion, T_e, j, v_iy, n_a_new, α0, ζ, kI, ε_dim, v_a, h)
        smooth_field!(E_z, 5)
        ν_m_grid = ν_m0 ./ max.(T_e, 1e-12).^(3/2)
        thrust_step = move_particles!(particles, E_y, E_y, E_z, E_z, H_x_half, H_x_half, j, j,
                                     ν_m_grid, ν_m_grid, x_grid, x_half, τ, h, ε, ν_m0, v_a, kI, n_a_new, H0_func)
        push!(thrust_time, t+τ); push!(thrust_values, thrust_step / τ)
...
let
    H0_func(z) = 1.0
    run_simulation(L=1.0, M=200, N1=200, total_time=2.0, save_times=[2.0], do_plot=true, kR=0.0)
end